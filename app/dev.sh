...
#   ./dev.sh reload   copy the .py sources into dist/BrainAI.app and relaunch via `open`
...
sync_py() { cp brainai.py brainai_server.py mcp_server.py updater.py update_ui.py env.default VERSION "$RES/"; codesign --force --sign - "$APP" >/dev/null 2>&1 || true; }
