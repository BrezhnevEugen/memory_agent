...
- Settings is split into **General**, **Projects** and **Readme** tabs (Readme: first-5-minutes guide, how projects work, manual MCP config). **Projects**: registry (`projects.json`) with a display name, an immutable id and the linked folders of each project. **New…** creates a project, **Link folder…** writes the project-scoped MCP configs for Claude Code (`.mcp.json`), Cursor (`.cursor/mcp.json`) and Codex (`.codex/config.toml`) into a folder in one go, **Unlink** removes them. Any number of folders can share one project. **Claude Desktop →** binds the global Claude Desktop config to the selected project
